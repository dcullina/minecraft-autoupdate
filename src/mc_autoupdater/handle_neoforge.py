...
def get_latest_version(main_version: int, sub_version: int) -> str:
	...
	filtered_versions = [version for version in versions if version.startswith(f"{main_version}.{sub_version}")]
...
def upgrade_neoforge(version: str) -> None:
	...
	if response.status_code == 200:
		...
		try:
			result = subprocess.run(["java", "-jar", file_name, "--installClient"], check=True)
